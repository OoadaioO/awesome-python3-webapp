...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        # 构造默认SELECT,INSERT,UPDATE,DELETE语句
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def update(self):
        ...
        if rows != 1:
            logging.warning('fail to update record:affected rows:%s' % rows)

    async def remove(self):
        args = [self.get_value_or_default(self.__primary_key__)]
        rows = await execute(self.__delete__, args)
        if rows != 1:
            logging.warning('fail to delete record:affected rows:%s' % rows)
